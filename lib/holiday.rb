...
def all_supplies_in_holidays(holiday_hash)
  ...
  holiday_hash.keys.each do |season|
    ...
    holiday_hash[season].each do |holiday|
      holiday_supplies = []
      holiday[1].each do |supply|
        holiday_supplies << supply
      end
      holiday_array = holiday[0].to_s.split("_")
      holiday_array.each do |word|
        word.capitalize!
      end
      holiday[0] = holiday_array.join(" ")
      puts "  #{holiday[0]}: #{holiday_supplies.join(", ")}"
    end
  end
end
