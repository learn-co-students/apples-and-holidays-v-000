...
def add_supply_to_memorial_day(holiday_hash, supply)
  ...
  holiday_hash[:spring].each {|holiday, item| item << supply}
end
...
def all_supplies_in_holidays(holiday_hash)
  ...
  holiday_hash.each do |season, holiday|
    puts "#{season.to_s.capitalize!}:"
      ...
      puts "  #{holiday_day.to_s.split("_").map {|word| word.capitalize!}.join(" ")}: #{items.join(", ")}"
    end
  end
end
...
def all_holidays_with_bbq(holiday_hash)
  ...
  holiday_hash.each do |season, holiday|
    holiday.each do |holiday_day, items|
        all_holidays_with_bbq << holiday_day if items.include?("BBQ")
    end
  end
  all_holidays_with_bbq
end
