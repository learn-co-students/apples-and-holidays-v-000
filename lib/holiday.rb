...
def all_supplies_in_holidays(holiday_hash)
  ...
  # etc.
  holiday_supplies.each do |season, supply|
  puts "#{season.to_s.capitalize!}:"
  supply.each do |holiday, char|
    holiday_names = holiday.to_s.split("_").collect {|word| word.capitalize!}.join(" ")
    puts"  #{holiday_names}: #{char.join(", ")}"
  end
end
end
