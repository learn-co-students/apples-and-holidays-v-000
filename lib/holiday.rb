...
def all_supplies_in_holidays(holiday_hash)
  ...
  # etc.
  holiday_hash.each do |season, hash1|
    puts "#{season.to_s.capitalize}:"
    hash1.each do |holiday, supply|
      new_holiday = holiday.to_s.split('_')
      new_holiday.each {|word| word.capitalize!}
      puts "  #{new_holiday.join(' ')}: #{supply.join(", ")}"
    end
  end
end
...
def all_holidays_with_bbq(holiday_hash)
  ...
  holiday_hash.each do |season, holiday_hash_2|
    holiday_hash_2.each do |holiday, supply_array|
