...
def all_supplies_in_holidays(holiday_hash)
  ...
  holiday_hash.each do |season, holiday_data|
    ...
    holiday_data.each do |holiday, supplies_array|
      better_holiday = []
      holiday.to_s.split("_").each { |word| better_holiday << word.capitalize! }
      puts "  #{better_holiday.join(" ")}: #{supplies_array.join(", ")}"
    end
  end
...
def all_holidays_with_bbq(holiday_hash)
  ...
  holiday_hash.each do |season, holiday_data|
    holiday_data.each do |holiday, supplies|
      holiday_bbq_names << holiday if supplies.include?("BBQ")
    end
  end
  holiday_bbq_names
end
