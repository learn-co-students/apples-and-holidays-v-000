...
def all_supplies_in_holidays(holiday_hash)
  ...
  holiday_hash.each do |season, holiday_name|
    ...
    holiday_name.each do |holiday, supply_array|
      formatted_holiday_name = []
      formatted_holiday_name<<holiday.to_s.split("_")
      formatted_holiday_name.flatten!
        formatted_holiday_name.map! do |item|
          item.capitalize!
        end
        formatted_holiday_name = formatted_holiday_name.join(" ")
        supply_array = supply_array.join(", ")
        puts "  #{formatted_holiday_name}: #{supply_array}"
    end
  end
end
