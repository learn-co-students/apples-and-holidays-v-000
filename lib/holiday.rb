...
def all_supplies_in_holidays(holiday_hash)
  ...
  # etc.
  holiday_hash.each {|season, holiday_supplies_hash|
    puts "#{season.capitalize}:"
    holiday_supplies_hash.each {|holiday, supplies|
...
def all_holidays_with_bbq(holiday_hash)
  ...
  array = []
  holiday_hash.each {|season, holiday_supplies_hash|
    holiday_supplies_hash.each { |holiday, holiday_supplies|
      holiday_supplies.each { |x|
        if x == "BBQ"
          array << holiday_hash[season].keys
        end
      }
    }
  }
