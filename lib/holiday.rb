...
def all_winter_holiday_supplies(holiday_hash)
  ...
  #holiday_supplies[:winter].collect do |holiday, items|
    #items
  #end.flatten 
  ...
  holiday_hash.each do |season, holiday_data|
    puts "#{season.to_s.capitalize}:"
      holiday_data.each do |holiday, items|
        holiday_name = holiday.to_s.split("_").collect {|words| words.capitalize}.join(" ") 
...
def all_holidays_with_bbq(holiday_hash)
  ...
  holiday_hash.collect do |season, holiday_data|
    holiday_data.collect do |holiday, items|
