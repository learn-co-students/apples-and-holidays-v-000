...
def all_supplies_in_holidays(holiday_hash)
  ...
  holiday_hash.each do |season, data|
    ...
    data.each do |holiday, supplies|
      ...
      holiday_str.capitalize!
      if holiday_str.include? "_"
        holiday_array = holiday_str.split("_")
        holiday_array_new = []
        holiday_array.each do |holiday_word|
          holiday_word.capitalize!
          holiday_array_new << holiday_word
        end
        holiday_str = holiday_array_new.join(" ")
      end
...
def all_holidays_with_bbq(holiday_hash)
  ...
  holiday_hash.each do |season, data|
    data.each do |holiday, supplies|
      supply_str = supplies.to_s
      if supply_str.include? "BBQ"
        bbq_array << holiday
      end
    end
  end
