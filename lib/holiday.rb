...
def all_supplies_in_holidays(holiday_hash)
  ...
  holiday_hash.each do |season, holidays|
    ...
    holidays.each do |holiday, supplies|
      if supplies.size == 2
        puts "  #{holiday.to_s.split("_").each {|capital| capital.capitalize!}.join(" ")}: #{supplies[0]}, #{supplies[1]}"
      elsif supplies.size == 1
        puts "  #{holiday.to_s.split("_").each {|capital| capital.capitalize!}.join(" ")}: #{supplies[0]}"
      end
    end
  end
