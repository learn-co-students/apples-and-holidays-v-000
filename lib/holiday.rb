...
def add_new_holiday_with_supplies(holiday_hash, season, holiday_name, supply_array)
  ...
  holiday_hash.each do |season_name, value|
    if season_name == season
      value.merge!(holiday_name => supply_array)
    end
  end
end
...
def all_supplies_in_holidays(holiday_hash)
  ...
  holiday_hash.each do |season, value_hash|
    ...
    value_hash.each do |holiday, supply_array|
      holiday_string = holiday.to_s
      holiday_cap = holiday_string.split('_').map(&:capitalize).join(' ')

      supply_list = supply_array.join(', ')
      puts "  #{holiday_cap}: #{supply_list}"
    end
  end
end
